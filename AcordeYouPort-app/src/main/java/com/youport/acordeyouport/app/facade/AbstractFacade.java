...
public abstract class AbstractFacade<T> {
    ...
    public <T> T listUniqueNamedQuery(Class<T> entityType, String namedQuery, HashMap<String, String> parameters) {
        ...
        /**
     * Retorna la entidad resultante de la ejecución del query En caso de error
     * retorna null
     *
     * @param <T>
     * @param entityType
     ...
     * @return
     */
    public <T> T uniqueNamedQuery(Class<T> entityType, String namedQuery, HashMap<String, Object> parameters) {
        T result;
        Query query = getEntityManager().createNamedQuery(namedQuery);
        for (String nameParameter : parameters.keySet()) {
            query.setParameter(nameParameter, parameters.get(nameParameter));
    ...
    /**
     * Retorna un String resultante de la ejecución del query En caso de error
     ...
     * @return
     */
    public T uniqueCreateQuery(String CreateQuery, HashMap<String, Object> parameters) {
        T result;
        try {
            Query query = getEntityManager().createQuery(CreateQuery);
            for (String nameParameter : parameters.keySet()) {
                query.setParameter(nameParameter, parameters.get(nameParameter));
            }
            result = (T) query.getSingleResult();
        } catch (Exception e) {
//            LOG4J.error(" Error ejecución de query - ", e);
            result = null;
        }
